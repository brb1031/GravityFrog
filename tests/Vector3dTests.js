...
*/
	...
	function testLength(vector, expectedLength){
		if(Math.isClose(vector.getMagnitude(), expectedLength)){
			return true;
		} else {
	...
	function testEquality(vector, expectedVector){
		if (vector.isClose(expectedVector)){
			return true;
		}else if (testLength(vector, expectedVector.getMagnitude())){
	...
	function testCoAxial(v1, v2){
		var cosOpeningAngle = v1.dot(v2) / (v1.getMagnitude() * v2.getMagnitude());
		var v1Dotv2 = v1.dot(v2);
		v1Dotv2 *= v1Dotv2;
	...
	while(specialVectors.length){
		...
		v.expectedLength = v.getMagnitude();
		vectors.push(v);
		unitVectors.push(v);
	}
	...
	for (i=0; i < numSamples; i ++){
		...
		if (!Math.isClose(Math.cos(u.getTheta()), Math.cos(theta))){
			console.log("Theta (" + theta + ") not recovered from unitFromAngles, got " + u.getTheta() + " instead.");
			passed = false;
		}else if (!Math.isClose(u.getPhi()*Math.sin(theta), phi*Math.sin(theta))){
			console.log("Phi (" + phi + ")not recovered from unitFromAngles, got " + u.getPhi() + " instead.");
